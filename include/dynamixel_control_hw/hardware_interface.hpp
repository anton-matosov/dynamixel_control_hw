...
namespace dynamixel {
    ...
    **/
    ...
    private:
        ...
        // The following should only be specified in the config file for supported servos

        // Map for compliance margin, bilateral (ID: margin)
        std::unordered_map<id_t, int> _dynamixel_margin;
        // Map for compliance slope, bilateral (ID: slope)
        std::unordered_map<id_t, int> _dynamixel_slope;
        // Map for punch (ID: punch)
        std::unordered_map<id_t, int> _dynamixel_punch;
        // Map for position p gain (ID: p gain)
        std::unordered_map<id_t, int> _dynamixel_p_gain;
        // Map for position i gain (ID: i gain)
        std::unordered_map<id_t, int> _dynamixel_i_gain;
        // Map for position d gain (ID: d gain)
        std::unordered_map<id_t, int> _dynamixel_d_gain;
        ...
        ros::NodeHandle& root_nh, ros::NodeHandle& robot_hw_nh)
    {
        ...
        // Search for the servos declared by the user, in the parameters
        ...
        const ros::Duration& elapsed_time)
    {
        for (unsigned i = 0; i < _servos.size(); i++) {
            ...
            if (status.valid()) {
                ...
                //ROS_INFO_STREAM(_servos[i]->id() << ": " << _joint_angles[i]);
            }
            else {
        ...
        if (got_all_params &= robot_hw_nh.getParam("servos", servos_param)) {
            ...
            try {
                for (XmlRpc::XmlRpcValue::ValueStruct::const_iterator it = servos_param.begin(); it != servos_param.end(); ++it) {
                    ...
                    if (it->second.hasMember("margin")) {
                        _dynamixel_margin[id]
                            = static_cast<int>(servos_param[it->first]["margin"]);
                        ROS_DEBUG_STREAM("\tmargin: "
                            << _dynamixel_margin[id]);
                    }
                    if (it->second.hasMember("slope")) {
                        _dynamixel_slope[id]
                            = static_cast<int>(servos_param[it->first]["slope"]);
                        ROS_DEBUG_STREAM("\tslope: "
                            << _dynamixel_slope[id]);
                    }
                    if (it->second.hasMember("punch")) {
                        _dynamixel_punch[id]
                            = static_cast<int>(servos_param[it->first]["punch"]);
                        ROS_DEBUG_STREAM("\tpunch: "
                            << _dynamixel_punch[id]);
                    }
                    if (it->second.hasMember("p_gain")) {
                        _dynamixel_p_gain[id]
                            = static_cast<int>(servos_param[it->first]["p_gain"]);
                        ROS_DEBUG_STREAM("\tp_gain: "
                            << _dynamixel_p_gain[id]);
                    }
                    if (it->second.hasMember("i_gain")) {
                        _dynamixel_i_gain[id]
                            = static_cast<int>(servos_param[it->first]["i_gain"]);
                        ROS_DEBUG_STREAM("\ti_gain: "
                            << _dynamixel_i_gain[id]);
                    }
                    if (it->second.hasMember("d_gain")) {
                        _dynamixel_d_gain[id]
                            = static_cast<int>(servos_param[it->first]["d_gain"]);
                        ROS_DEBUG_STREAM("\td_gain: "
                            << _dynamixel_d_gain[id]);
                    }
                }
            }
    ...
    **/
        ...
        dynamixel_servo servo, OperatingMode mode)
    {
        try {
            ...
            // Set compliance margin for actuators it is specified for, otherwise leave it as default

            typename std::unordered_map<id_t, int>::iterator
                dynamixel_margin_iterator
                = _dynamixel_margin.find(servo->id());
            // if found in the map, it has been set in the configuration
            if (dynamixel_margin_iterator != _dynamixel_margin.end()) {
              ROS_DEBUG_STREAM("Setting compliance margin of servo "
                  << _dynamixel_map[servo->id()] << " to "
                  << dynamixel_margin_iterator->second);
              //set the cw compliance margin
              _dynamixel_controller.send(
                    servo->set_cw_compliance_margin(
                      dynamixel_margin_iterator->second));
              _dynamixel_controller.recv(status);

//              //set return delay time to zero
//              _dynamixel_controller.send(
//                    servo->set_return_delay_time(0));
//              _dynamixel_controller.recv(status);

              //set the ccw compliance margin
              _dynamixel_controller.send(
                    servo->set_ccw_compliance_margin(
                      dynamixel_margin_iterator->second));
              _dynamixel_controller.recv(status);
            }

            // Set compliance slope for actuators it is specified for, otherwise leave it as default

            typename std::unordered_map<id_t, int>::iterator
                dynamixel_slope_iterator
                = _dynamixel_slope.find(servo->id());
            // if found in the map, it has been set in the configuration
            if (dynamixel_slope_iterator != _dynamixel_slope.end()) {
              ROS_DEBUG_STREAM("Setting compliance slope of servo "
                  << _dynamixel_map[servo->id()] << " to "
                  << dynamixel_slope_iterator->second);
              //set the cw compliance slope
              _dynamixel_controller.send(
                    servo->set_cw_compliance_slope(
                      dynamixel_slope_iterator->second));
              _dynamixel_controller.recv(status);
              //set the ccw compliance slope
              _dynamixel_controller.send(
                    servo->set_ccw_compliance_slope(
                      dynamixel_slope_iterator->second));
              _dynamixel_controller.recv(status);
            }

            // Set punch for actuators it is specified for, otherwise leave it as default

            typename std::unordered_map<id_t, int>::iterator
                dynamixel_punch_iterator
                = _dynamixel_punch.find(servo->id());
            // if found in the map, it has been set in the configuration
            if (dynamixel_punch_iterator != _dynamixel_punch.end()) {
              ROS_DEBUG_STREAM("Setting punch of servo "
                  << _dynamixel_map[servo->id()] << " to "
                  << dynamixel_punch_iterator->second);
              //set the punch
              _dynamixel_controller.send(
                    servo->set_punch(
                      dynamixel_punch_iterator->second));
            ...
            // Set p_gain for actuators it is specified for, otherwise leave it as default

            typename std::unordered_map<id_t, int>::iterator
                dynamixel_p_gain_iterator
                = _dynamixel_p_gain.find(servo->id());
            // if found in the map, it has been set in the configuration
            if (dynamixel_p_gain_iterator != _dynamixel_p_gain.end()) {
              ROS_DEBUG_STREAM("Setting p_gain of servo "
                  << _dynamixel_map[servo->id()] << " to "
                  << dynamixel_p_gain_iterator->second);
              //set the p_gain
              _dynamixel_controller.send(
                    servo->set_position_p_gain(
                      dynamixel_p_gain_iterator->second));
              _dynamixel_controller.recv(status);
            }

            // Set i_gain for actuators it is specified for, otherwise leave it as default

            typename std::unordered_map<id_t, int>::iterator
                dynamixel_i_gain_iterator
                = _dynamixel_i_gain.find(servo->id());
            // if found in the map, it has been set in the configuration
            if (dynamixel_i_gain_iterator != _dynamixel_i_gain.end()) {
              ROS_DEBUG_STREAM("Setting i_gain of servo "
                  << _dynamixel_map[servo->id()] << " to "
                  << dynamixel_i_gain_iterator->second);
              //set the i_gain
              _dynamixel_controller.send(
                    servo->set_position_i_gain(
                      dynamixel_i_gain_iterator->second));
              _dynamixel_controller.recv(status);
            }

            // Set d_gain for actuators it is specified for, otherwise leave it as default

            typename std::unordered_map<id_t, int>::iterator
                dynamixel_d_gain_iterator
                = _dynamixel_d_gain.find(servo->id());
            // if found in the map, it has been set in the configuration
            if (dynamixel_d_gain_iterator != _dynamixel_d_gain.end()) {
              ROS_DEBUG_STREAM("Setting d_gain of servo "
                  << _dynamixel_map[servo->id()] << " to "
                  << dynamixel_d_gain_iterator->second);
              //set the d_gain
              _dynamixel_controller.send(
                    servo->set_position_d_gain(
                      dynamixel_d_gain_iterator->second));
              _dynamixel_controller.recv(status);
            }
        }
